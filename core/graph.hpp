...
class Graph {
public:
  int partition_id; // 通过MPI_Comm_rank设置
  int partitions; // part数量, 通过MPI_Comm_size设置

  size_t alpha;

  int threads; //总共cpu数量; 通过numa_num_configured_cpus()获取 ; 
  int sockets; //总共node数量; 通过numa_num_configured_nodes()获取
  int threads_per_socket; //每个node上的cpu数量; threads / sockets

  size_t edge_data_size; // 一条边属性的字节大小
  size_t unit_size; 
  size_t edge_unit_size; //一条边的字节大小(2个顶点+1条边属性大小)

  bool symmetric; // 是否对称
  VertexId vertices;  //顶点数量
  EdgeId edges; //边数量
  ...
  // 所有part的偏移量; 大小取决于partitions, 计算方法:remained_amount / remained_partitions
  VertexId * partition_offset; // VertexId [partitions+1]
  // 本地part的偏移量,大小取决于sockets
  ...
  // 是一个大小为sockets的数组,每个元素是一个大小为vertices的bitmap
  ...
  // 无向图=compressed_outgoing_adj_vertices
  ...
  void load_directed(std::string path, VertexId vertices) {
    ...
    symmetric = false; //是否对称

    MPI_Datatype vid_t = get_mpi_data_type<VertexId>();

    this->vertices = vertices; // 顶点数量
    long total_bytes = file_size(path.c_str()); //文件大小
    this->edges = total_bytes / edge_unit_size; //边的数量
    #ifdef PRINT_DEBUG_MESSAGES
    if (partition_id==0) {
      printf("|V| = %u, |E| = %lu\n", vertices, edges);
    }
    ...
    EdgeId read_edges = edges / partitions; // 每个part读的边数量
    if (partition_id==partitions-1) {
      read_edges += edges % partitions;
    }
    long bytes_to_read = edge_unit_size * read_edges; // 每个part读的字节数
    long read_offset = edge_unit_size * (edges / partitions * partition_id); // 要读的偏移量
    long read_bytes; //实际已经读到字节
    int fin = open(path.c_str(), O_RDONLY); // 打开文件
    EdgeUnit<EdgeData> * read_edge_buffer = new EdgeUnit<EdgeData> [CHUNKSIZE]; //读到的边

    // 计算出每个顶点的出度 out_degree
    out_degree = alloc_interleaved_vertex_array<VertexId>(); // 每个顶点的出度
    for (VertexId v_i=0;v_i<vertices;v_i++) {
      out_degree[v_i] = 0;
    }
    assert(lseek(fin, read_offset, SEEK_SET)==read_offset);
    read_bytes = 0;
    while (read_bytes < bytes_to_read) {
      long curr_read_bytes; // 本次读的字节
      if (bytes_to_read - read_bytes > edge_unit_size * CHUNKSIZE) {
        // 读取到的边在read_edge_buffer中
        curr_read_bytes = read(fin, read_edge_buffer, edge_unit_size * CHUNKSIZE);
      } else {
        curr_read_bytes = read(fin, read_edge_buffer, bytes_to_read - read_bytes);
      }
      assert(curr_read_bytes>=0);
      read_bytes += curr_read_bytes;
      EdgeId curr_read_edges = curr_read_bytes / edge_unit_size; // 本次读的边数量
      #pragma omp parallel for         // 并行计算出度out_degree
      for (EdgeId e_i=0;e_i<curr_read_edges;e_i++) {
        VertexId src = read_edge_buffer[e_i].src;
        VertexId dst = read_edge_buffer[e_i].dst;
        __sync_fetch_and_add(&out_degree[src], 1);// 出度计算,没有排除重复边记录
      }
    }
    // 对out_degree做聚合
    MPI_Allreduce(MPI_IN_PLACE, out_degree, vertices, vid_t, MPI_SUM, MPI_COMM_WORLD);
    ...
    for (int i=0;i<partitions;i++) {
      ...
      EdgeId expected_chunk_size = remained_amount / remained_partitions;
      if (remained_partitions==1) {
        partition_offset[i+1] = vertices; //最后一个偏移量是顶点的总数量
      } else {
        EdgeId got_edges = 0;
        for (VertexId v_i=partition_offset[i];v_i<vertices;v_i++) {
          got_edges += out_degree[v_i] + alpha;
          if (got_edges > expected_chunk_size) {
            partition_offset[i+1] = v_i; // 每个part的partition_offset取决于chunk_size=remained_amount / remained_partitions
            break;
          }
        }
        partition_offset[i+1] = (partition_offset[i+1]) / PAGESIZE * PAGESIZE; // aligned with pages
    ...
    // 计算 local_partition_offset
    {
      ...
      for (int s_i=0;s_i<sockets;s_i++) {
        ...
        EdgeId expected_chunk_size = remained_amount / remained_partitions;
        if (remained_partitions==1) {
          // 本地part最后一个socket的偏移量为下一个part的偏移量
          local_partition_offset[s_i+1] = partition_offset[partition_id+1];
        } else {
          EdgeId got_edges = 0;
          for (VertexId v_i=local_partition_offset[s_i];v_i<partition_offset[partition_id+1];v_i++) {
  ...
  std::function<void(VertexId, VertexAdjList<EdgeData>)> dense_signal, std::function<R(VertexId, M)> dense_slot, Bitmap * active, Bitmap * dense_selective = nullptr) {
    ...
    if (sparse) {
      ...
      // 遍历当前part的所有活跃顶点,并调用sparse_signal
      ...
      // 收集local_send_buffer到send_buffer
      #pragma omp parallel for
      for (int t_i=0;t_i<threads;t_i++) {
        flush_local_send_buffer<M>(t_i);
      }
